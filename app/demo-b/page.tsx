...
export default function DemoB() {
  ...
  // 마운트 후 Ken Burns 시작 트리거
  useEffect(() => {
    // requestAnimationFrame으로 브라우저 paint 후 class 추가해 transition 발동
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      bgRef.current.style.transform = `translateY(${scrollY * 0.4}px) scale(${
        mounted ? 1.0 : 1.08
      })`;
    };
  ...
  return (
    <>
      ...
      >
        {/* 배경 이미지 — Ken Burns zoom-out + 패럴랙스 */}
        <div
          ref={bgRef}
          style={{
            position: "absolute",
            ...
            // Ken Burns: 초기 scale(1.08) → mounted 시 scale(1.0)
            transform: `scale(${mounted ? 1.0 : 1.08})`,
            transition: mounted
        ...
        <div
          style={{
            ...
            textAlign: "center",
            // Ken Burns 텍스트 fade-in: 0.5초 딜레이, 1.2초
            opacity: mounted ? 1 : 0,
            transform: mounted ? "translateY(0)" : "translateY(20px)",
            transition: mounted
              ? "opacity 1.2s ease 0.5s, transform 1.2s ease 0.5s"
              : "none",
          }}
